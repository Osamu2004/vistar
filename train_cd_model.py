...
from cdcore.trainer.cd_trainer import CDTrainer
...
def main():
    ...
    run_config = setup.setup_run_config(config, CDRunConfig)

    model = create_cd_model(config["net_config"]["name"],dataset = config['data_provider'].get('type'),)
    print(model)
