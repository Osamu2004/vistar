...
class  TFM1(nn.Module):
    ...
    def forward(self, inputs) -> torch.Tensor:
        ...
        x = fusion1+fusion2+fusion3
        x = self.project(x)
        return self.act(self.norm(x))


class  TFM_3D_11(nn.Module):
    def __init__(self, int_ch: int, norm=("ln2d",)):
        super().__init__()
        self.diff_conv_spatial = nn.Conv3d(
            int_ch, int_ch, kernel_size=(2, 3, 3), 
            padding=(0, 1, 1), groups=int_ch
        )
        self.diff_conv_channel = nn.Conv2d(int_ch, int_ch, kernel_size=1)
        self.norm = build_norm(norm[0], int_ch)
        self.act = build_act("gelu")

    def forward(self, x1: torch.Tensor, x2: torch.Tensor) -> torch.Tensor:
        fusion = torch.stack([x1, x2], dim=2)
        x = self.diff_conv_spatial(fusion).squeeze(2)
        x = self.diff_conv_channel(x)
