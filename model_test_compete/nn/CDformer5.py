...
from layer.nn.ops import LinearLayer,ResidualBlock,DualResidualBlock,IdentityLayer,TimeMBConv,LayerScale2d,build_norm,MBConv
...
class LowMixer(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., pool_size=2,
        **kwargs, ):
        ...
        self.kernel_func = build_act("relu", inplace=False)
    ...
    def relu_linear_att(self, q, k, v, B, N, C) -> torch.Tensor:
        ...
        if out.dtype == torch.bfloat16:
            out = out.float()
        out = out[:, :, :-1] / (out[:, :, -1:] + self.eps).reshape(B, C, N)
